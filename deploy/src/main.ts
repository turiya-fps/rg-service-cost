...
import * as TerraformAppFeature from 'cdktf/lib/features';
...
const main = (): void => {
  ...
  const application = new TerraformApp({
    outdir,
    context: {
      [TerraformAppFeature.EXCLUDE_STACK_ID_FROM_LOGICAL_IDS]: true,
      [TerraformAppFeature.ALLOW_SEP_CHARS_IN_LOGICAL_IDS]: true,
    },
  });
  ...
  console.log(`${prefix} deploying stage ${chalk.green(variables.deploy.stage)} on environemnt ${chalk.green(variables.deploy.environment)}`);
