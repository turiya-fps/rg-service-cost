...
const resolveServiceDomainPurpose = (scope: Construct, provider: AccountWithProvider<AccountWithPurpose<AccountPurposeEnvironment>>): ServiceDomainPurpose | undefined => {
  ...
  return Object.freeze<ServiceDomainPurpose>({
    hostname: environment.parameter(
      'input-domain-brand-api-record-service-cost-hostname',
      '/rg/domain/brand/api/record/service/cost/hostname',
    ).value,
    ...
    certificate: {
      arn: environment.parameter(
        'input-domain-brand-api-record-service-cost-certificate-arn',
        '/rg/domain/brand/api/record/service/cost/certificate/arn',
      ).value,
    },
  });
};
...
export const resolveFoundationStageInputs = (scope: Construct, providers: FoundationStageProviders): FoundationStageInputMapping => {
  ...
  return {
    domain: {
      codename: {
        hostname: environment.parameter(
          'input-domain-codename-env-record-service-cost-hostname',
          '/rg/domain/codename/env/record/service/cost/hostname',
        ).value,
        ...
        certificate: {
          arn: environment.parameter(
            'input-domain-codename-env-record-service-cost-certificate-arn',
            '/rg/domain/codename/env/record/service/cost/certificate/arn',
          ).value,
        },
      },
