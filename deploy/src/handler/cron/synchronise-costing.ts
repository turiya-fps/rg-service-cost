...
import { LambdaHandler } from '@project-rouge/infrastructure-core/stack/composition/lambda/lambda-handler';
import type { HandlerEnvironmentMapping } from '@project-rouge/service-cost/src/handler/cron/synchronise-costing/entrypoint';
import { Construct } from 'constructs';
import type { SnowflakeCredentialSecretMapping } from '../../stage/application/input';
...
export class SynchroniseCostingCronHandlerDefinition extends Construct {
  public readonly handler: LambdaHandler<HandlerEnvironmentMapping>;
  ...
  public constructor(scope: Construct, props: HandlerDefinitionProps) {
    ...
    const operation = 'synchronise-costing';
    ...
    this.handler = new LambdaHandler<HandlerEnvironmentMapping>(this, 'handler', {
      providers: {
      ...
      source: {
        ...
        entrypoint: `handler/${kind}/${operation}/entrypoint.handler`,
      ...
      environment: {
        ...
        SNOWFLAKE_ACCOUNT: props.external.snowflake.account,
        SNOWFLAKE_USERNAME: props.external.snowflake.username,
        SNOWFLAKE_PASSWORD: props.external.snowflake.password,
        SNOWFLAKE_WAREHOUSE: props.external.snowflake.warehouse,
        SNOWFLAKE_ROLE: props.external.snowflake.role,
      },
    });
