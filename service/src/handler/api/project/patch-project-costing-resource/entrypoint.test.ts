...
describe('route', (): void => {
  it('method', (): void => {
    expect(route.method).toStrictEqual<PatchProjectCostingResource.Method>('PATCH');
  });

  it('path, lambda syntax', (): void => {
    expect(route.path(lambda)).toStrictEqual<string>('/projects/{projectId}/costing');
  });

  it('path, express syntax', (): void => {
    expect(route.path(express)).toStrictEqual<string>('/projects/:projectId/costing');
  });
});
