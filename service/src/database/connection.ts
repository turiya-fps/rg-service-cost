...
import type { DateFactory, DateValue } from '@project-rouge/service-core/data/date';
...
export const createDatabaseConnectionFactory = (
  ...
  return async (date: DateValue): Promise<DatabaseConnection> => {
...
/**
 * Create a {@link DatabaseConnectionManager} supporting the current environment.
 *
 * - {@link createDataSourceForSocket()}
 * - {@link createDataSourceForRoleAuthentication()}
 */
export const createDatabaseConnectionManager = async (
  date: DateFactory,
  runtime: RuntimeType,
  environment: Partial<EnvironmentMapping>,
): Promise<DatabaseConnectionManager> => {
  const factory = createDatabaseConnectionFactory(runtime, environment);

  return new DatabaseConnectionManager(factory, date);
};
